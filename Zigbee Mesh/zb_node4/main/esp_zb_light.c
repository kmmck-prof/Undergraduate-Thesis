...
void ReportAttribute (uint16_t attr_id){
	...
	esp_zb_zcl_attr_t *value_r = esp_zb_zcl_get_attribute(ZB_CLIENT_ENDPOINT_4, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, attr_id);
...
void Data_Complete_Task (void *params){
	while(1){
		if(uxSemaphoreGetCount(semaphore) == TASK_COUNT){
			...
			snprintf(data2, sizeof(data2), "K%.0f,%.2f,%.7f,%.7f,%.2f,%s", no2_score, no2, lat_deg, long_deg, speed_kmh, Current_Date_Time);
...
static void esp_zb_task(void *pvParameters)
{
    ...
    //Set Endpoint HERE
    esp_zb_ep_list_add_ep(esp_zb_ep_list, esp_zb_cluster_list, ZB_CLIENT_ENDPOINT_4, ESP_ZB_AF_HA_PROFILE_ID, ESP_ZB_HA_ON_OFF_SWITCH_DEVICE_ID);
