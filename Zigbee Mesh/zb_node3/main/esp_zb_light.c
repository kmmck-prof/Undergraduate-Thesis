...
static float pm25, pm10, rh, temp, co, no2, co2, voc = 0;
int no2_adc, co_adc;
float pm25_score, pm10_score, co_score, no2_score, co2_score, voc_score, iaq = 0;
char* location[4];
...
static const char *TAG = "ESP_ZB_CLIENT_3";
...
void ReportAttribute (uint16_t attr_id){
	...
	report_here.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_3;
	...
	esp_zb_zcl_attr_t *value_r = esp_zb_zcl_get_attribute(ZB_CLIENT_ENDPOINT_3, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, attr_id);
...
void ReadAttribute (void){
	...
	read_here.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_3;
...
void sd_card_write(void){
    ...
    if (first_write == 1){
    	...
    	//snprintf(log, MAX_SIZE, "Time, CO2 (ppm), CO2 Index, VOC (ppb), VOC Index, PM2.5 (ug/m3), PM2.5 Score, PM10 (ug/m3), PM10 Score, T (C), RH (%s)\n", "%%");
    ...
    //snprintf(log, MAX_SIZE, "%s, %.2f, %.0f, %.2f, %.0f, %.2f, %.0f, %.2f, %.0f, %.2f, %.2f,\n", Current_Date_Time, co2, co2_score, voc, voc_score, pm25, pm25_score, pm10, pm10_score, temp, rh);
    //printf("%s\n", log);
...
void process_sen55(uint8_t data[24]){
	...
	temp = temp_raw / 200;
	//Node 3
	pm25 = 1.94396684659239*pm25 + (3.430952855605721);
	pm10 = 2.326756692376159*pm10 + (2.141467096886128);
	temp = 0.5543796022251568*temp + (10.202591845421477);
	rh = 1.0126905301066023*rh + (-13.636454040580918);
}
...
/*         configure report attribute command*/
        ...
        report_cmd.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_3;
...
static void esp_zb_task(void *pvParameters)
{
    ...
    //Set Endpoint HERE
    esp_zb_ep_list_add_ep(esp_zb_ep_list, esp_zb_cluster_list, ZB_CLIENT_ENDPOINT_3, ESP_ZB_AF_HA_PROFILE_ID, ESP_ZB_HA_ON_OFF_SWITCH_DEVICE_ID);
...
void app_main(void)
{
    ...
    vTaskDelay(12500/portTICK_PERIOD_MS);
