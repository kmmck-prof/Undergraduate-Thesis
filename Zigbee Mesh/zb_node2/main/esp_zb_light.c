...
TaskHandle_t sen_handle, mics_handle, dummy, sgp_handle, finish_handle, uart_handle, zb_handle, scd_handle;
...
float co2_scd41_1, co2_scd41_2, temp2, rh2;
int no2_adc, co_adc;
...
double lat_deg = 0;
double long_deg = 0;
int longitude_decimal = 0;
int latitude_decimal = 0;
char* speed_str = "0";
...
static const char *TAG = "ESP_ZB_CLIENT_2";
...
void zb_report_attribute(void){
    esp_zb_zcl_report_attr_cmd_t attr_report;
	attr_report.zcl_basic_cmd.dst_addr_u.addr_short = zb_coord_info.short_addr;
	attr_report.zcl_basic_cmd.dst_endpoint = zb_coord_info.endpoint;
	attr_report.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_2;
	attr_report.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
	attr_report.clusterID = SENSOR_DATA_CLUSTER_ID;
	attr_report.cluster_role = ESP_ZB_ZCL_CLUSTER_SERVER_ROLE; //SERVER ACTUALLY MEANS I HAVE THE DATA, NOT CLIENT WILL SEND DATA
	attr_report.attributeID = SENSOR_DATA_ATTR_ID;
    //char *new_value = "This sentence is at least 100 characters. Limit testing 50 characters only. This is the new updated ESP-ZIGBEE-SDK Library!\0";
    char data[150] = "150,1000.00,150,1000.00 100.00 100.00 150 5000 150 60000 150 10000";
    esp_zb_zcl_set_attribute_val(ZB_CLIENT_ENDPOINT_2, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, SENSOR_DATA_ATTR_ID, &data, false);

   // esp_zb_zcl_attr_t *value_r = esp_zb_zcl_get_attribute(ZB_CLIENT_ENDPOINT_2, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, SENSOR_DATA_ATTR_ID);
    //memcpy(value_r->data_p, &data, sizeof(data));
    //esp_zb_zcl_report_attr_cmd_req(&attr_report);
    vTaskDelay(100/portTICK_PERIOD_MS);
    char data2[150] = "K,150,1000.00,150,1000.00,00.000000,000.000000,00.00,2024-00-00T00:00:00";
    attr_report.attributeID = SENSOR_DATA_ATTR_ID_2;
    esp_zb_zcl_attr_t *value_r2 = esp_zb_zcl_get_attribute(ZB_CLIENT_ENDPOINT_2, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, SENSOR_DATA_ATTR_ID_2);
    memcpy(value_r2->data_p, &data2, sizeof(data2));
    //esp_zb_zcl_report_attr_cmd_req(&attr_report);
}

void zb_report_attribute2(void){
    esp_zb_zcl_report_attr_cmd_t attr_report;
	attr_report.zcl_basic_cmd.dst_addr_u.addr_short = zb_coord_info.short_addr;
	attr_report.zcl_basic_cmd.dst_endpoint = zb_coord_info.endpoint;
	attr_report.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_2;
	...
	attr_report.attributeID = SENSOR_DATA_ATTR_ID;
    //char *new_value = "This sentence is at least 100 characters. Limit testing 50 characters only. This is the new updated ESP-ZIGBEE-SDK Library!\0";
    char data[150] = "K,150 1000.00 150 1000.00 00.000000,000.000000,00.00 2024-00-00T00:00:00";
    esp_zb_zcl_set_attribute_val(ZB_CLIENT_ENDPOINT_2, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, SENSOR_DATA_ATTR_ID, &data, false);
}

void reporting_task(void *pvParams){
	printf("Setting up periodic reports...\n");
    ...
    report_cmd.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_2;
    ...
    report_cmd.reportable_change = "";
    esp_zb_zcl_config_report_cmd_req(&report_cmd);
    while(1){
    	printf("Reporting...\n");
        zb_report_attribute();
        vTaskDelay(3000/portTICK_PERIOD_MS);
        zb_report_attribute2();
    }
}
...
void ReportAttribute (uint16_t attr_id){
	...
	report_here.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_2;
	...
	esp_zb_zcl_attr_t *value_r = esp_zb_zcl_get_attribute(ZB_CLIENT_ENDPOINT_2, SENSOR_DATA_CLUSTER_ID, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, attr_id);
...
void ReadAttribute (void){
	...
	read_here.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_2;
...
void sd_card_write(void){
    ...
    if (first_write == 1){
    	//Write Headers
    	//snprintf(log, MAX_SIZE, "Time, CO (ppb), CO Index, CO2 (ppm), CO2 Index, NO2 (ppb), NO2 Index, PM2.5 (ug/m3), PM2.5 Index, PM10 (ug/m3), PM10 Index, VOC (ppb), VOC Index, Temperature (C), RH (%%)\n");
    	snprintf(log, MAX_SIZE,"Time, SCD41-1 (CO2), SCD41-2 (CO2)\n");
    ...
    snprintf(log, MAX_SIZE, "%s, %.2f, %.2f\n", Current_Date_Time, co2_scd41_1, co2_scd41_2);
...
void process_scd41(uint8_t data[9]){
	//Raw Bit Data
	//CO2 Bits
	uint16_t co2_bits = data[0] << 8; //First 8 bits
			 co2_bits |= data[1]; //Add last 8 bits
	uint16_t temp_bits = data[3] << 8;
			 temp_bits |= data[4];
	uint16_t rh_bits = data[6] << 8;
			 rh_bits |= data[7];
	float temp_raw = temp_bits;
	temp = -45 + (175*temp_raw)/pow(2,16);
	float rh_raw = rh_bits;
	rh = 100 * rh_raw / pow(2,16);
...
void process_scd41_2(uint8_t data[9]){
	//Raw Bit Data
	//CO2 Bits
	uint16_t co2_bits = data[0] << 8; //First 8 bits
			 co2_bits |= data[1]; //Add last 8 bits
	...
	uint16_t rh_bits = data[6] << 8;
			 rh_bits |= data[7];
	float temp_raw = temp_bits;
	temp2 = -45 + (175*temp_raw)/pow(2,16);
	float rh_raw = rh_bits;
	rh2 = 100 * rh_raw / pow(2,16);
...
void I2C1_Data_Task(void *params){
	...
	vTaskDelay(1000/portTICK_PERIOD_MS);
	uint8_t SCD41_SERIAL[2] = {0x36, 0x82};
	uint8_t SCD41_PERIODIC_MEAS[2] = {0x21, 0xb1};
	uint8_t SCD41_READ_MEAS[2] = {0xec, 0x05}; //Get Measured Values Command
    uint8_t scd41_data[10];
	i2c_master_write_to_device(I2C_PORT_1, SCD41_ADDR, SCD41_PERIODIC_MEAS, 2, 10/portTICK_PERIOD_MS); //Measurement Mode
	vTaskDelay(10/portTICK_PERIOD_MS);
	while (1) {
    	//For SEN55 (Starts at Idle Mode)
    	i2c_master_write_to_device(I2C_PORT_1, SCD41_ADDR, SCD41_READ_MEAS, 2, 1000/portTICK_PERIOD_MS); //Send Get-Measurement Command
    	vTaskDelay(20/portTICK_PERIOD_MS);
    	i2c_master_read_from_device(I2C_PORT_1, SCD41_ADDR, scd41_data, 9, 1000/portTICK_PERIOD_MS); //Read Measurements
    	vTaskDelay(10/portTICK_PERIOD_MS);
    	//process_sen55(sen55_data); //Update Global Variables
    	process_scd41(scd41_data);
    	//printf("%d", uxTaskGetStackHighWaterMark(NULL));
    	printf("SCD41_1 - CO2: %.2f, T: %.2f, RH: %.2f\n", co2_scd41_1, temp, rh);
        xSemaphoreGive(semaphore);
        vTaskSuspend(sen_handle);
        vTaskDelay(5000/portTICK_PERIOD_MS);
    }
}
void SCD41_Data_Task(void *params){
	//SEN55 Initialize
	i2c_config_t conf1 = {
			.mode = I2C_MODE_MASTER,
	        .sda_io_num = I2C0_SDA,
	...
	i2c_param_config(I2C_PORT_0, &conf1);
	i2c_driver_install(I2C_PORT_0, conf1.mode, 0, 0, 0);
	vTaskDelay(1000/portTICK_PERIOD_MS);
	uint8_t SCD41_SERIAL[2] = {0x36, 0x82};
	uint8_t SCD41_PERIODIC_MEAS[2] = {0x21, 0xb1};
	uint8_t SCD41_READ_MEAS[2] = {0xec, 0x05}; //Get Measured Values Command
    uint8_t scd41_data[10];
	i2c_master_write_to_device(I2C_PORT_0, SCD41_ADDR, SCD41_PERIODIC_MEAS, 2, 10/portTICK_PERIOD_MS); //Measurement Mode
	vTaskDelay(10/portTICK_PERIOD_MS);
	while (1) {
    	//For SEN55 (Starts at Idle Mode)
    	i2c_master_write_to_device(I2C_PORT_0, SCD41_ADDR, SCD41_READ_MEAS, 2, 1000/portTICK_PERIOD_MS); //Send Get-Measurement Command
    	vTaskDelay(20/portTICK_PERIOD_MS);
    	i2c_master_read_from_device(I2C_PORT_0, SCD41_ADDR, scd41_data, 9, 1000/portTICK_PERIOD_MS); //Read Measurements
    	vTaskDelay(10/portTICK_PERIOD_MS);
    	//process_sen55(sen55_data); //Update Global Variables
    	process_scd41_2(scd41_data);
    	//printf("%d", uxTaskGetStackHighWaterMark(NULL));
    	printf("SCD41_2 - CO2: %.2f, T: %.2f, RH: %.2f\n", co2_scd41_2, temp2, rh2);
        xSemaphoreGive(semaphore);
        vTaskSuspend(scd_handle);
        vTaskDelay(5000/portTICK_PERIOD_MS);
    }
}
...
void Data_Complete_Task (void *params){
	while(1){
		if(uxSemaphoreGetCount(semaphore) == TASK_COUNT){
			...
			//snprintf(data2, sizeof(data2), "%.0f,%.2f,%.0f,%.2f,%.0f,%.2f,%.2f,%.2f", voc_score, voc, co_score, co, no2_score, no2, lat_deg, long_deg); //OUTDOOR VERSION
			printf("%s, %.2f, %.2f\n", Current_Date_Time, co2_scd41_1, co2_scd41_2);
...
static void user_find_cb(esp_zb_zdp_status_t zdo_status, uint16_t addr, uint8_t endpoint, void *user_ctx)
{
    if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS) {
        ...
        read_req.clusterID = SENSOR_DATA_CLUSTER_ID;
        read_req.attributeID = SENSOR_DATA_ATTR_ID;
        read_req.zcl_basic_cmd.dst_endpoint = zb_coord_info.endpoint;
        read_req.zcl_basic_cmd.src_endpoint = ZB_CLIENT_ENDPOINT_2;
        read_req.zcl_basic_cmd.dst_addr_u.addr_short = zb_coord_info.short_addr;
...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_STEERING:
        if (err_status != ESP_OK) {
            ...
            xTaskCreate(reporting_task, "Attribute Report", 1024*4, NULL, 5, NULL);
...
static void esp_zb_task(void *pvParameters)
{
    ...
    //Set Endpoint HERE
    esp_zb_ep_list_add_ep(esp_zb_ep_list, esp_zb_cluster_list, ZB_CLIENT_ENDPOINT_2, ESP_ZB_AF_HA_PROFILE_ID, ESP_ZB_HA_ON_OFF_SWITCH_DEVICE_ID);
...
void app_main(void)
{
	...
	//xTaskCreate(I2C1_Data_Task, "I2C1 Data Task", 1024*3, NULL, 5, &sen_handle);
	//xTaskCreate(SCD41_Data_Task, "SCD41 Data Task", 1024*3, NULL, 5, &scd_handle);
	//xTaskCreate(UART_Data_Task, "UART Data Task", 1024*5, NULL, 5, &uart_handle);
	vTaskDelay(1000/portTICK_PERIOD_MS);
