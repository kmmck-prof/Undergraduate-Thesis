...
//OpenThread
#define TAG "OpenThread Outdoor Node"
#define UDP_PORT 2222
// #define UDP_PORT 4444 // designated UDP port in BR for node 4
...
void Data_Complete_Task (void *params){
	while(1){
		...
		// THREAD
			snprintf(data_payload, 650, "{\"COraw\":\"%.2f\",\"COindex\":\"%.0f\",\"CO2raw\":\"%.2f\",\"CO2index\":\"%.0f\",\"NO2raw\":\"%.2f\",\"NO2index\":\"%.0f\",\"PM25raw\":\"%.2f\",\"PM25index\":\"%.0f\",\"PM10raw\":\"%.2f\",\"PM10index\":\"%.0f\",\"VOCraw\":\"%.2f\",\"VOCindex\":\"%.0f\",\"T\":\"%.2f\",\"RH\":\"%.2f\",\"source\":\"Node 4\",\"local_time\":\"%s\",\"type\":\"data\"}",co, co_score, co2, co2_score, no2, no2_score, pm25, pm25_score,pm10,pm10_score,voc, voc_score, temp, rh, Rx_Date_Time);
			udp_send();
			snprintf(data_payload, 200, "{\"Lat\":\"%.7f\",\"Long\":\"%.7f\",\"speed\":\"%.2f\",\"source\":\"Node 4\",\"local_time\":\"%s\",\"type\":\"data\"}", lat_deg, long_deg, speed_kmh, Rx_Date_Time);
			udp_send();
...
void app_main(void)
{
    ...
    vTaskDelay(37500/portTICK_PERIOD_MS);
	sensor_init(&heated);
